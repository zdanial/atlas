...
export class ApiAdapter implements StorageAdapter {
  ...
  private async request<T>(
    ...
    if (!res.ok) {
      ...
      throw new Error(
        (errorBody as Record<string, string>).error ?? `HTTP ${res.status}`,
      );
    }
  ...
  async getNode(id: string): Promise<Node | null> {
    try {
      const data = await this.request<{ node: Record<string, unknown> }>(
        `/api/nodes/${id}`,
      );
  ...
  async createNode(input: CreateNode): Promise<Node> {
    const data = await this.request<{ node: Record<string, unknown> }>(
      '/api/nodes',
      { method: 'POST', body: JSON.stringify(toCreateBody(input)) },
    );
    return toNode(data.node);
  }
  ...
  async updateNode(id: string, patch: Partial<Node>): Promise<Node> {
    const data = await this.request<{ node: Record<string, unknown> }>(
      `/api/nodes/${id}`,
      { method: 'PATCH', body: JSON.stringify(toUpdateBody(patch)) },
    );
    return toNode(data.node);
  }
  ...
  async logAgentRun(input: CreateAgentRun): Promise<AgentRun> {
    const data = await this.request<{ agent_run: Record<string, unknown> }>(
      '/api/agent-runs',
      {
        method: 'POST',
        body: JSON.stringify({
